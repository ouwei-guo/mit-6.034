...
def get_large_nn():
    ...
    for p in na:
        for s in p:
            t.append(s)
    t.append('Z')
    nn = NeuralNet(['x', 'y', -1], t)

    
    #ten layers of 10 neurons
    for n1 in na[0]:
        nn.join('x', n1, w()).join('y', n1, w())
        
    for i in range(len(na)-1):
        for j in range(len(na[i])):
            for m in range(len(na[i][j])):
                nn.join(na[i][j], na[i+1][m], w())
...
def start_training(data=None, net=None, resolution=None):
    ...
    try:
        nn = multi_back_prop(nn, train, 3, -0.001, resolution)
...
if __name__ == "__main__":
    """horizontal', 'diagonal', 'stripe', 'checkerboard',
    'letterL', 'moat'"""
    train = "stripe"
    net = "medium"
    resolution = 5
